...
import { DISCORD_EMBED_LIMIT, DISCORD_FILE_LIMIT, DISCORD_TOTAL_LIMIT } from "./constants"
import { EmbedQueueData, Env } from './types'
import { getAuthHeader, getDiscordWebhook, sleep } from "./utils"

/** Sends multiple embeds with no .txt fallback */
export async function sendDiscordEmbeds(messages: EmbedQueueData[], env: Env, ctx: ExecutionContext) {
	const nextSize = 0 // max = DISCORD_TOTAL_LIMIT
	const embeds = []
...
/** Sends single embed with .txt fallback */
